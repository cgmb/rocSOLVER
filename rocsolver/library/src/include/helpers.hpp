...
// ROCSOLVER_UNREACHABLE_X is a variant of ROCSOLVER_UNREACHABLE that takes a string as a second
// parameter, which should explain why this path is believed to be unreachable.
#define ROCSOLVER_UNREACHABLE_X(invariant, message) ROCSOLVER_UNREACHABLE(invariant) 
...
#ifdef ROCSOLVER_VERIFY_ASSUMPTIONS
#define ROCSOLVER_ASSUME(invariant) do \
{                                      \
  if(!(invariant))                     \
    std::abort();                      \
} while(0)
...
// which should explain what is supposed to guarantee this invariant.
