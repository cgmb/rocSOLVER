...
{
    size_t s1, s2, unused;
    rocsolver_orml2_unml2_getMemorySize<T, BATCHED>(side, m, n, batch_count, size_1, size_2, size_3,
                                                    size_4);
    ...
    if(k > ORMLQ_ORML2_BLOCKSIZE)
    {
        ...
        rocsolver_larfb_getMemorySize<T, BATCHED>(side, m, n, min(jb, k), batch_count, &s2, &unused,
                                                  size_5);
                                              ...
                                              T* workTrmm)
{
    ...
    if(k <= ORMLQ_ORML2_BLOCKSIZE)
        return rocsolver_orml2_unml2_template<T>(handle, side, trans, m, n, k, A, shiftA, lda,
                                                 strideA, ipiv, strideP, C, shiftC, ldc, strideC,
                                                 batch_count, scalars, work, workArr, trfact);
    ...
    for(rocblas_int j = 0; j < k; j += ldw)
    {
        ...
        rocsolver_larft_template<T>(handle, rocblas_forward_direction, rocblas_row_wise, order - i,
                                    min(ldw, k - i), A, shiftA + idx2D(i, i, lda), lda, strideA,
        ...
        rocsolver_larfb_template<BATCHED, STRIDED, T>(
            handle, side, transB, rocblas_forward_direction, rocblas_row_wise, nrow, ncol,
            min(ldw, k - i), A, shiftA + idx2D(i, i, lda), lda, strideA, trfact, 0, ldw, strideW, C,
            shiftC + idx2D(ic, jc, ldc), ldc, strideC, batch_count, work, workArr, workTrmm);
    }
