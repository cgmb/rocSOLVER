...
void rocsolver_larf_getMemorySize(const rocblas_int m,
                                  const rocblas_int n,
                                       ...
                                       T** workArr)
{
    ...
    rocblasCall_gemv<T>(handle, trans, m, n, cast2constType<T>(scalars), 0, A, shiftA, lda, stridea,
                        x, shiftx, incx, stridex, cast2constType<T>(scalars + 1), 0, work, 0, 1,
                        order, batch_count, workArr);

    // compute the rank-1 update  (A + tau*V*W'  or A + tau*W*V')
    if(leftside)
    {
        rocblasCall_ger<COMPLEX, T>(handle, m, n, alpha, stridep, x, shiftx, incx, stridex, work, 0,
                                    1, order, A, shiftA, lda, stridea, batch_count, workArr);
    }
    else
    {
        rocblasCall_ger<COMPLEX, T>(handle, m, n, alpha, stridep, work, 0, 1, order, x, shiftx,
