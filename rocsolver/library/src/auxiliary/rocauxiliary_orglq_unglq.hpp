...
__global__ void set_zero_row(const rocblas_int m,
                                              ...
                                              T* workTrmm)
{
    ...
    if(kk < m)
    {
        blocksx = (m - kk - 1) / 32 + 1;
        blocksy = (kk - 1) / 32 + 1;
        hipLaunchKernelGGL(set_zero_row<T>, dim3(blocksx, blocksy, batch_count), dim3(32, 32), 0,
                           stream, m, kk, A, shiftA, lda, strideA);
    ...
    while(j >= 0)
    {
        ...
        if(j + jb < m)
        {
            ...
            rocsolver_larfb_template<BATCHED, STRIDED, T>(
                ...
                shiftA + idx2D(j + jb, j, lda), lda, strideA, batch_count, work, workArr, workTrmm);
        }
        ...
        if(j > 0)
        {
            blocksx = (jb - 1) / 32 + 1;
            blocksy = (j - 1) / 32 + 1;
            hipLaunchKernelGGL(set_zero_row<T>, dim3(blocksx, blocksy, batch_count), dim3(32, 32),
                               0, stream, j + jb, j, A, shiftA, lda, strideA);
        }
        rocsolver_orgl2_ungl2_template<T>(handle, jb, n - j, jb, A, shiftA + idx2D(j, j, lda), lda,
                                          strideA, (ipiv + j), strideP, batch_count, scalars, work,
                                          workArr);
