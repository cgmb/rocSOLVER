...
__global__ void copyshift_col(const bool copy,
...
template <typename T, typename U>
__global__ void copyshift_row(const bool copy,
                              const rocblas_int dim,
                              U A,
                              const rocblas_int shiftA,
                              const rocblas_int lda,
                                         ...
                                         size_t* size_5)
{
    if(storev == rocblas_column_wise)
    {
        if(m >= k)
        {
            rocsolver_orgqr_ungqr_getMemorySize<T, BATCHED>(m, n, k, batch_count, size_1, size_2,
        ...
        else
        {
            ...
            rocsolver_orgqr_ungqr_getMemorySize<T, BATCHED>(m - 1, m - 1, m - 1, batch_count,
                                                            size_1, &s2, size_3, size_4, size_5);
            *size_2 = max(s1, s2);
        }
    }
    else
    {
        if(n > k)
        {
            rocsolver_orglq_unglq_getMemorySize<T, BATCHED>(m, n, k, batch_count, size_1, size_2,
                                                            size_3, size_4, size_5);
        }
        else
        {
                                              ...
                                              T* workTrmm)
{
    ...
    if(storev == rocblas_column_wise)
    {
        ...
        else
        {
            ...
            // copy
            hipLaunchKernelGGL(copyshift_col<T>, dim3(blocks, blocks, batch_count), dim3(BS, BS), 0,
            ...
            rocsolver_orgqr_ungqr_template<BATCHED, STRIDED, T>(
                ...
                strideP, batch_count, scalars, work, workArr, trfact, workTrmm);
        }
    }
    ...
    else
    {
        ...
        else
        {
            ...
            // copy
            hipLaunchKernelGGL(copyshift_row<T>, dim3(blocks, blocks, batch_count), dim3(BS, BS), 0,
