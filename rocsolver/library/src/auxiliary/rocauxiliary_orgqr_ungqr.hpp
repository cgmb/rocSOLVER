...
__global__ void set_zero_col(const rocblas_int n,
                                              ...
                                              T* workTrmm)
{
    ...
    if(kk < n)
    {
        blocksx = (kk - 1) / 32 + 1;
        blocksy = (n - kk - 1) / 32 + 1;
        hipLaunchKernelGGL(set_zero_col<T>, dim3(blocksx, blocksy, batch_count), dim3(32, 32), 0,
                           stream, n, kk, A, shiftA, lda, strideA);
    ...
    while(j >= 0)
    {
        ...
        if(j > 0)
        {
            blocksx = (j - 1) / 32 + 1;
            blocksy = (jb - 1) / 32 + 1;
            hipLaunchKernelGGL(set_zero_col<T>, dim3(blocksx, blocksy, batch_count), dim3(32, 32),
