                                         ...
                                         size_t* size_5)
{
    ...
    if(storev == rocblas_column_wise)
        rocsolver_ormqr_unmqr_getMemorySize<T, BATCHED>(side, m, n, min(nq, k), batch_count, size_1,
                                                        size_2, size_3, size_4, size_5);
    else
        rocsolver_ormlq_unmlq_getMemorySize<T, BATCHED>(side, m, n, min(nq, k), batch_count, size_1,
                                                        size_2, size_3, size_4, size_5);
}
    ...
    if(storev == rocblas_column_wise)
    {
        if(nq >= k)
        {
            rocsolver_ormqr_unmqr_template<BATCHED, STRIDED, T>(
                ...
                ldc, strideC, batch_count, scalars, work, workArr, trfact, workTrmm);
        }
        else
        {
