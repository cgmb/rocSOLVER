...
{
    size_t s1, s2, unused;
    rocsolver_orm2r_unm2r_getMemorySize<T, BATCHED>(side, m, n, batch_count, size_1, size_2, size_3,
                                                    size_4);
                                              ...
                                              T* workTrmm)
{
    ...
    if(k <= ORMQR_ORM2R_BLOCKSIZE)
        return rocsolver_orm2r_unm2r_template<T>(handle, side, trans, m, n, k, A, shiftA, lda,
    ...
    for(rocblas_int j = 0; j < k; j += ldw)
    {
        ...
        rocsolver_larft_template<T>(handle, rocblas_forward_direction, rocblas_column_wise,
                                    order - i, min(ldw, k - i), A, shiftA + idx2D(i, i, lda), lda,
        ...
        rocsolver_larfb_template<BATCHED, STRIDED, T>(
            handle, side, trans, rocblas_forward_direction, rocblas_column_wise, nrow, ncol,
