    ...
    else
    {
        *size_4 = sizeof(rocblas_int) * batch_count;
        ...
        if(uplo == rocblas_fill_upper)
            rocblasCall_trsm_mem<BATCHED, T>(rocblas_side_left, jb, n - jb, batch_count, size_5,
                                             size_6, size_7, size_8);
        else
            rocblasCall_trsm_mem<BATCHED, T>(rocblas_side_right, n - jb, jb, batch_count, size_5,
                                             size_6, size_7, size_8);
    }
}
                                        ...
                                        bool optim_mem)
{
    ...
    if(n < POTRF_POTF2_SWITCHSIZE)
        return rocsolver_potf2_template<T>(handle, uplo, n, A, shiftA, lda, strideA, info,
                                           batch_count, scalars, work, pivotGPU);
    ...
    if(uplo == rocblas_fill_upper)
    {
        ...
        for(rocblas_int j = 0; j < n; j += POTRF_POTF2_SWITCHSIZE)
        {
            ...
            rocsolver_potf2_template<T>(handle, uplo, jb, A, shiftA + idx2D(j, j, lda), lda,
                                        strideA, iinfo, batch_count, scalars, work, pivotGPU);
            ...
            if(j + jb < n)
            {
                ...
                rocblasCall_trsm<BATCHED, T>(
                    ...
                    strideA, batch_count, optim_mem, x_temp, x_temp_arr, invA, invA_arr);
