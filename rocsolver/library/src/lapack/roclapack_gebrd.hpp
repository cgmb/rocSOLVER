                                        ...
                                        T* Abyx_norms)
{
    ...
    if(m <= k || n <= k)
        return rocsolver_gebd2_template<real_t<T>, T>(
    ...
    while(j < dim - k)
    {
        ...
        rocsolver_labrd_template<real_t<T>, T>(
            handle, m - j, n - j, jb, A, shiftA + idx2D(j, j, lda), lda, strideA, D + j, strideD,
            E + j, strideE, tauq + j, strideQ, taup + j, strideP, X, shiftX, ldx, strideX, Y,
            shiftY, ldy, strideY, batch_count, scalars, work_workArr, Abyx_norms);
    ...
    // factor last block
    if(j < dim)
        rocsolver_gebd2_template<real_t<T>, T>(handle, m - j, n - j, A, shiftA + idx2D(j, j, lda),
                                               lda, strideA, D + j, strideD, E + j, strideE,
                                               tauq + j, strideQ, taup + j, strideP, batch_count,
