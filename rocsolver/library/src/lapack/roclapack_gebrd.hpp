                                   ...
                                   size_t* size_6)
{
    ...
    else
    {
        ...
        rocsolver_gebd2_getMemorySize<T, BATCHED>(m - d * k, n - d * k, batch_count, size_1, size_2,
                                                  size_3, size_4);
                                        ...
                                        T* diag)
{
    ...
    if(m <= k || n <= k)
    {
        rocsolver_gebd2_template<S, T>(handle, m, n, A, shiftA, lda, strideA, D, strideD, E,
                                       strideE, tauq, strideQ, taup, strideP, batch_count, scalars,
                                       work, workArr, diag);
    ...
    while(j < dim - k)
    {
        ...
        rocsolver_labrd_template<S, T>(handle, m - j, n - j, jb, A, shiftA + idx2D(j, j, lda), lda,
                                       ...
                                       strideY, batch_count, scalars, work, workArr, diag);
        ...
        rocblasCall_gemm<BATCHED, STRIDED, T>(
            ...
            batch_count, workArr);
        rocblasCall_gemm<BATCHED, STRIDED, T>(
            handle, rocblas_operation_none, rocblas_operation_none, m - j - jb, n - j - jb, jb,
            &minone, X, shiftX + jb, ldx, strideX, A, shiftA + idx2D(j, j + jb, lda), lda, strideA,
            &one, A, shiftA + idx2D(j + jb, j + jb, lda), lda, strideA, batch_count, workArr);
    ...
    if(j < dim)
        rocsolver_gebd2_template<S, T>(handle, m - j, n - j, A, shiftA + idx2D(j, j, lda), lda,
                                       strideA, D + j, strideD, E + j, strideE, tauq + j, strideQ,
                                       taup + j, strideP, batch_count, scalars, work, workArr, diag);
