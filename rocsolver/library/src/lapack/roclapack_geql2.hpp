...
{
    size_t s1, s2;
    rocsolver_larf_getMemorySize<T, BATCHED>(rocblas_side_left, m, n, batch_count, size_1, &s1,
                                             size_3);
                                        ...
                                        T* diag)
{
    ...
    for(rocblas_int j = 0; j < dim; j++)
    {
        ...
        rocsolver_larfg_template(handle, m - j, A, shiftA + idx2D(m - j - 1, n - j - 1, lda), A,
                                 ...
                                 (ipiv + dim - j - 1), strideP, batch_count, diag, work);
        ...
        rocsolver_larf_template(handle, rocblas_side_left, m - j, n - j - 1, A,
                                ...
                                strideP, A, shiftA, lda, strideA, batch_count, scalars, work,
                                workArr);
