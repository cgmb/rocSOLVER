                                   ...
                                   size_t* size_9)
{
    ...
    else
    {
        *size_4 = sizeof(rocblas_int) * batch_count;
        ...
        rocblasCall_trsm_mem<BATCHED, T>(rocblas_side_left, jb, n - jb, batch_count, size_6, size_7,
                                         size_8, size_9);
    }
}
                                        ...
                                        bool optim_mem)
{
    ...
    if(m < GETRF_GETF2_SWITCHSIZE || n < GETRF_GETF2_SWITCHSIZE)
        return rocsolver_getf2_template<ISBATCHED, T>(handle, m, n, A, shiftA, lda, strideA, ipiv,
                                                      ...
                                                      scalars, pivot_val, pivot_idx, work);
    ...
    for(rocblas_int j = 0; j < dim; j += GETRF_GETF2_SWITCHSIZE)
    {
        ...
        rocsolver_getf2_template<ISBATCHED, T>(handle, m - j, jb, A, shiftA + idx2D(j, j, lda), lda,
                                               strideA, ipiv, shiftP + j, strideP, iinfo, batch_count,
                                               pivot, scalars, pivot_val, pivot_idx, work);
