                                            ...
                                            const rocblas_int batch_count)
{
    ...
    rocsolver_gebrd_getMemorySize<T, true>(m, n, batch_count, &size_1, &size_2, &size_3, &size_4,
                                           &size_5, &size_6);
    ...
    hipMalloc(&X, size_5);
    hipMalloc(&Y, size_6);
    if(!scalars || (size_2 && !work) || (size_3 && !workArr) || (size_4 && !diag) || (size_5 && !X)
       || (size_6 && !Y))
    ...
    hipLaunchKernelGGL(get_array, dim3(blocks, 1, 1), dim3(32, 1, 1), 0, stream, (T**)XArr, (T*)X,
                       strideX, batch_count);
    hipLaunchKernelGGL(get_array, dim3(blocks, 1, 1), dim3(32, 1, 1), 0, stream, (T**)YArr, (T*)Y,
                       strideY, batch_count);
    ...
    rocblas_status status = rocsolver_gebrd_template<true, false, S, T>(
        ...
        lda, strideA, D, strideD, E, strideE, tauq, strideQ, taup, strideP, (U)XArr,
        0, // the matrix is shifted 0 entries (will work on the entire matrix)
        m, strideX, (U)YArr,
        0, // the matrix is shifted 0 entries (will work on the entire matrix)
        n, strideY, batch_count, (T*)scalars, (T*)work, (T**)workArr, (T*)diag);
