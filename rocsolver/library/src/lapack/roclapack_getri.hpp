                                   ...
                                   size_t* size_6)
{
    ...
    *size_1 = sizeof(T) * 3;
    ...
    if(n <= GETRI_SWITCHSIZE_LARGE)
        ...
        rocblasCall_trsm_mem<BATCHED, T>(rocblas_side_right, n, GETRI_BLOCKSIZE, batch_count,
    ...
    work_trsm
        = (n <= GETRI_SWITCHSIZE_MID ? n : n * GETRI_BLOCKSIZE) * sizeof(T) * batch_count + invA;
                                        ...
                                        bool optim_mem)
{
    ...
    if(A1 == nullptr) // in-place trtri
    {
        ...
        rocblasCall_trtri<BATCHED, STRIDED, T>(handle, rocblas_fill_upper, rocblas_diagonal_non_unit,
                                               n, A, shiftA, lda, strideA, work, 0, ldw, strideW,
        ...
        hipLaunchKernelGGL((getri_trtri_update<false, true, T>), dim3(batch_count, blocks, blocks),
                           dim3(1, 32, 32), 0, stream, n, A, shiftA, lda, strideA, work, 0, ldw,
                           strideW, info);
    }
    else // out-of-place trtri
    {
        ...
        rocblasCall_trtri<BATCHED, STRIDED, T>(
            handle, rocblas_fill_upper, rocblas_diagonal_non_unit, n, A1, shiftA1, lda1, strideA1,
            A, shiftA, lda, strideA, batch_count, (T*)x_temp, (T**)x_temp_arr, workArr);
    ...
    else
    {
        ...
        for(rocblas_int j = nn - 1; j >= 0; j -= nb)
        {
            ...
            hipLaunchKernelGGL(getri_kernel_large1<T>, dim3(batch_count, 1, 1), dim3(1, threads, 1),
                               0, stream, n, j, jb, A, shiftA, lda, strideA, info, work, strideW);

            if(j + jb < n)
                rocblasCall_gemm<BATCHED, STRIDED>(
                    handle, rocblas_operation_none, rocblas_operation_none, n, jb, n - j - jb,
                    &minone, A, shiftA + idx2D(0, j + jb, lda), lda, strideA, work, j + jb, ldw,
            ...
            rocblasCall_trsm<BATCHED, T>(handle, rocblas_side_right, rocblas_fill_lower,
                                         ...
                                         work, j, ldw, strideW, A, shiftA + idx2D(0, j, lda), lda,
                                         strideA, batch_count, optim_mem, x_temp, x_temp_arr,
                                         work + strideW * batch_count, invA_arr, workArr);
        }
