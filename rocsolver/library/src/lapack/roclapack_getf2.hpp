    ...
    for(rocblas_int j = 0; j < dim; ++j)
    {
        if(pivot)
            ...
            rocblasCall_iamax<ISBATCHED, T, S>(handle, m - j, A, shiftA + idx2D(j, j, lda), 1,
                                               strideA, batch_count, pivot_idx, work);
        ...
        hipLaunchKernelGGL(getf2_check_singularity<T>, dim3(batch_count), dim3(1), 0, stream, A,
                           shiftA, strideA, ipiv, shiftP, strideP, j, lda, pivot_val, pivot_idx,
                           info, pivot);
        ...
        rocblasCall_scal<T>(handle, m - j - 1, pivot_val, 1, A, shiftA + idx2D(j + 1, j, lda), 1,
                            strideA, batch_count);
