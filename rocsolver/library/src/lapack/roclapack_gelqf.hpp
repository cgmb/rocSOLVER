                                        ...
                                        T* trfact)
{
    ...
    while(j < dim - GEQRF_GEQR2_SWITCHSIZE)
    {
        ...
        if(j + jb < m)
        {
            ...
            rocsolver_larfb_template<BATCHED, STRIDED, T>(
                ...
                trfact, 0, ldw, strideW, A, shiftA + idx2D(j + jb, j, lda), lda, strideA,
                batch_count, work, workArr, diag);
        }
    ...
    // factor last block
    if(j < dim)
        rocsolver_gelq2_template<T>(handle, m - j, n - j, A, shiftA + idx2D(j, j, lda), lda, strideA,
                                    (ipiv + j), strideP, batch_count, scalars, work, workArr, diag);
