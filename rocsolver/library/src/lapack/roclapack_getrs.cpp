                                    ...
                                    const rocblas_int ldb)
{
    ...
    rocsolver_getrs_getMemorySize<false, T>(n, nrhs, batch_count, &size_1, &size_2, &size_3, &size_4);
    ...
    if((size_1 && !x_temp) || (size_2 && !x_temp_arr) || (size_3 && !invA) || (size_4 && !invA_arr))
        return rocblas_status_memory_error;

    // execution
    rocblas_status status = rocsolver_getrs_template<false, T>(
        handle, trans, n, nrhs, A, 0, lda, strideA, ipiv, strideP, B, 0, ldb, strideB, batch_count,
